...
/** Controller for the Inventory's Add Product form. */
public class AddProductController implements Initializable {
    ...
    /** Method for adding parts from the Inventory to a product's list of associated parts.
     Accomplished using specified object methods.
    ...
    /** Method for adding a new part to the Inventory.
     ...
     Advanced for loop is utilized to save parts to the new Product's associated parts list.
    ...
    public void onActionSaveProduct(ActionEvent event) throws IOException {
        ...
        int saveId = Inventory.getProductIdCounter();
        String saveName = "";
        int saveInv = 0;
        double savePrice = 0;
        int saveMax = 0;
        ...
        if (exceptCount == 0){
            Inventory.addProduct(new Product(saveId, saveName, savePrice, saveInv, saveMin, saveMax));
    ...
    /** Method to initialize the Add Product Form view tables with data from the Inventory's ObservableList of parts, and establish a new Product's list of associated parts.
     Sorted by ID number.
     */
    @Override
